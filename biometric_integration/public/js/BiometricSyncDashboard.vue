<template>
  <div class="bsd-root">
    ...
    <div class="bsd-header">
      ...
      <div class="bsd-header-right">
        <div v-if="activeTab === 'daily'" class="bsd-date-control">
          ...
          <input type="date" v-model="syncDate" @change="loadAll" class="bsd-date-input" />
        </div>
    ...
    <div class="bsd-tab-bar">
      <button :class="['bsd-tab', { active: activeTab === 'daily' }]"
        @click="activeTab = 'daily'">
        Daily View
      </button>
      <button :class="['bsd-tab', { active: activeTab === 'unit' }]"
      ...
      <template v-else-if="dashData">
        ...
        <div class="bsd-kpi-row">
          ...
          <div class="bsd-kpi-card"
            ...
            :title="isEtime ? 'Raw tab-delimited lines received from device' : 'Total raw punch events received from device'">
            <div class="bsd-kpi-icon punch">⬡</div>
            <div class="bsd-kpi-body">
              <div class="bsd-kpi-value">{{ (dashData.kpis.raw_records || 0).toLocaleString() }}</div>
          ...
          <div class="bsd-kpi-card"
            ...
            <div class="bsd-kpi-body">
              <div class="bsd-kpi-value">{{ (dashData.kpis.created || 0).toLocaleString() }}</div>
          ...
          <div class="bsd-kpi-card"
            ...
            <div class="bsd-kpi-body">
              <div class="bsd-kpi-value">{{ (dashData.kpis.skipped || 0).toLocaleString() }}</div>
              <div class="bsd-kpi-label">Duplicates Skipped</div>
              <div class="bsd-kpi-sub">{{ isEtime ? 'Checkin Records' : 'Punch Records' }}</div>
            </div>
          </div>

          <!-- Card 4: Unmapped employees -->
          <div class="bsd-kpi-card warn"
            ...
            <div class="bsd-kpi-body">
              <div class="bsd-kpi-value">{{ (dashData.kpis.unmapped_employees || 0).toLocaleString() }}</div>
          ...
          <div class="bsd-kpi-card error"
            ...
            <div class="bsd-kpi-body">
              <div class="bsd-kpi-value">{{ (dashData.kpis.errored || 0).toLocaleString() }}</div>
          ...
          <div class="bsd-kpi-card healthy"
            ...
            <div class="bsd-kpi-body">
              <div class="bsd-kpi-value">
                ...
                <span class="bsd-kpi-total">/ {{ dashData.kpis.locations_total }}</span>
              </div>
        ...
        <div class="bsd-kpi-legend">
          <span class="bsd-legend-group records">◼ {{ isEtime ? 'Employee' : 'Punch' }} Record metrics</span>
          <span class="bsd-legend-group employees">◼ Employee issue metrics</span>
          <span v-if="activeFilter" class="bsd-filter-badge" @click="activeFilter = null">
        ...
        <div class="bsd-card bsd-table-card">
          <div class="bsd-card-header">
            ...
            <div class="bsd-table-meta">
              {{ filteredRows.length }} of {{ dashData.table_rows.length }} units · {{ syncDate }}
            </div>
          </div>
          ...
          <div class="bsd-table-wrap">
            <table class="bsd-table">
              <thead>
                <tr>
                  ...
                  <th class="col-unit sortable" @click="sort('unit')">
                    ...
                    <sort-icon col="unit" :sort-key="sortKey" :sort-dir="sortDir" />
                  </th>
                  <th class="sortable" @click="sort('status')">
                    Status <sort-icon col="status" :sort-key="sortKey" :sort-dir="sortDir" />
                  </th>
                  <th class="sortable num-col" @click="sort('raw_records')"
                    title="Raw device records / lines received">
                    Raw Records
                    <sort-icon col="raw_records" :sort-key="sortKey" :sort-dir="sortDir" />
                  </th>
                  <th class="sortable num-col" @click="sort('total')"
                    :title="isEtime ? 'Unique employees in device response' : 'Total punch events received from device'">
                    {{ isEtime ? 'Employees' : 'Punches' }}
                    <sort-icon col="total" :sort-key="sortKey" :sort-dir="sortDir" />
                  </th>
                  <th class="sortable num-col" @click="sort('created')"
                    title="ERP Employee Checkin records created">
                    Checkins <sort-icon col="created" :sort-key="sortKey" :sort-dir="sortDir" />
                  </th>
                  <th class="sortable num-col" @click="sort('skipped')"
                    title="Duplicate checkins skipped">
                    Dupes <sort-icon col="skipped" :sort-key="sortKey" :sort-dir="sortDir" />
                  </th>
                  <th class="sortable num-col" @click="sort('unmapped_employees')"
                    title="Unique employees whose device ID is not mapped in ERP (employee count)">
                    Unmapped <sort-icon col="unmapped_employees" :sort-key="sortKey" :sort-dir="sortDir" />
                  </th>
                  <th class="sortable num-col" @click="sort('errored')"
                    title="Employees that matched but threw insert errors (employee count)">
                    Errored <sort-icon col="errored" :sort-key="sortKey" :sort-dir="sortDir" />
                  </th>
                  <th class="col-progress"
                    :title="`Clean rate: (${isEtime ? 'Employees' : 'Punches'} − Unmapped − Errored) / Total`">
                    Progress
                  </th>
              ...
              <tbody>
                <tr v-for="row in filteredRows" :key="row.unit"
                  :class="['bsd-row', `bsd-row-${row.status}`]">

                  <td class="col-dot"><span class="bsd-dot" :class="`dot-${row.status}`"></span></td>
                  ...
                  <td class="bsd-unit-cell">
                    ...
                    <a v-if="row.log_name"
                      ...
                      target="_blank"
                      class="bsd-log-link">{{ row.log_name }} ↗</a>
                  </td>
                  ...
                  <td class="num-col">
                    <span v-if="row.raw_records != null">{{ row.raw_records.toLocaleString() }}</span>
                    <span v-else class="bsd-nil">—</span>
                  </td>

                  <!-- employees_total -->
                  <td class="num-col">
                    <span v-if="row.total != null">{{ row.total.toLocaleString() }}</span>
                    <span v-else class="bsd-nil">—</span>
                  </td>

                  <!-- checkins_created -->
                  <td class="num-col">
                    <span v-if="row.created != null">{{ row.created.toLocaleString() }}</span>
                    <span v-else class="bsd-nil">—</span>
                  </td>

                  <!-- checkins_skipped (duplicates) -->
                  <td class="num-col">
                    <span v-if="row.skipped != null">{{ row.skipped.toLocaleString() }}</span>
                    <span v-else class="bsd-nil">—</span>
                  </td>

                  <!-- employees_not_found — employee count -->
                  <td class="num-col" :class="{ 'num-warn': (row.unmapped_employees || 0) > 0 }">
                    <span v-if="row.unmapped_employees != null">
                      {{ row.unmapped_employees.toLocaleString() }}
                      <span v-if="row.unmapped_employees > 0" class="num-unit">emp</span>
                    </span>
                    <span v-else class="bsd-nil">—</span>
                  </td>

                  <!-- errored_count -->
                  <td class="num-col" :class="{ 'num-error': (row.errored || 0) > 0 }">
                    <span v-if="row.errored != null">
                      {{ row.errored.toLocaleString() }}
                      <span v-if="row.errored > 0" class="num-unit">emp</span>
                    </span>
                    <span v-else class="bsd-nil">—</span>
                  </td>
                    ...
                    <template v-if="row.progress != null">
                      <div class="bsd-prog-wrap">
                        <div class="bsd-prog-bar">
                          <div class="bsd-prog-fill" :class="progressClass(row.progress)"
                            :style="{ width: row.progress + '%' }"></div>
                        </div>
                        <span class="bsd-prog-pct">{{ row.progress }}%</span>
                      </div>
                  ...
                  <td class="col-actions">
                    <button v-if="row.status === 'missing'"
                      ...
                      title="Trigger single-day sync for this date">Sync</button>
                    <button v-else-if="row.status === 'error'"
                      class="bsd-action-btn retry"
                      @click="doRetryApi(row)"
                      title="Re-fetch this date's data from the device (1 day only)">Retry API</button>
                    <button v-if="row.log_name && row.status !== 'missing'"
                      class="bsd-action-btn detail"
                      @click="openDrawer(row)"
                      title="View log details">Detail</button>
                  </td>
                </tr>

                <tr v-if="filteredRows.length === 0">
                  <td colspan="11" class="bsd-empty">
                    {{ activeFilter ? 'No units match the current filter.' : 'No sync data for this date.' }}
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div class="bsd-charts-row">
          <div class="bsd-card bsd-chart-card">
            ...
            <div v-if="!trendData?.labels?.length" class="bsd-chart-empty">No trend data available</div>
          </div>
          ...
          <div class="bsd-card bsd-chart-card">
            ...
            <div v-if="failureData?.length" class="bsd-failure-list">
              <div v-for="item in failureData" :key="item.category" class="bsd-failure-row">
                <span class="bsd-failure-label">{{ item.category }}</span>
                <div class="bsd-failure-bar-wrap">
                  <div class="bsd-failure-bar" :style="{ width: failureBarWidth(item.count) + '%' }"></div>
                </div>
    ...
    <template v-else-if="activeTab === 'unit'">
      ...
      <template v-else-if="unitHistory">
        ...
        <div class="bsd-unit-summary-bar">
          ...
          <div class="bsd-sum-stat">
            <div class="bsd-sum-value">{{ unitHistory.summary.total_raw_records.toLocaleString() }}</div>
            <div class="bsd-sum-label">Total Raw Records</div>
          </div>
          <div class="bsd-sum-stat" v-if="isEtime">
            <div class="bsd-sum-value">{{ unitHistory.summary.employees_total.toLocaleString() }}</div>
            <div class="bsd-sum-label">Total Employees</div>
          </div>
          <div class="bsd-sum-stat">
            <div class="bsd-sum-value">{{ unitHistory.summary.created.toLocaleString() }}</div>
        ...
        <div class="bsd-card bsd-table-card">
          <div class="bsd-card-header">
            ...
            <div class="bsd-table-meta">
              {{ unitHistory.rows.length }} days · {{ unitFromDate }} to {{ unitToDate }}
            </div>
          </div>
          <div class="bsd-table-wrap">
            <table class="bsd-table">
              <thead>
                <tr>
                  ...
                  <th>Date</th>
                  <th>Status</th>
                  <th class="num-col" title="Raw device records / lines received">Raw Records</th>
                  <th class="num-col" :title="isEtime ? 'Unique employees in response' : 'Punch events received'">
                    {{ isEtime ? 'Employees' : 'Punches' }}
                  </th>
                  <th class="num-col" title="ERP checkins created">Checkins</th>
                  <th class="num-col" title="Duplicate checkins skipped">Dupes</th>
                  <th class="num-col" title="Unmapped employees (employee count)">Unmapped</th>
                  <th class="num-col" title="Insert errors (employee count)">Errored</th>
                  <th class="col-progress">Progress</th>
                  <th class="col-actions">Log</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="row in unitHistory.rows" :key="row.sync_date"
                  :class="['bsd-row', `bsd-row-${row.status}`]">
                  <td class="col-dot">
                  ...
                  <td class="num-col">{{ (row.raw_records || 0).toLocaleString() }}</td>
                  <td class="num-col">{{ (row.total || 0).toLocaleString() }}</td>
                  <td class="num-col">{{ (row.created || 0).toLocaleString() }}</td>
                  <td class="num-col">{{ (row.skipped || 0).toLocaleString() }}</td>
                  <td class="num-col" :class="{ 'num-warn': (row.unmapped_employees || 0) > 0 }">
                ...
                <tr v-if="unitHistory.rows.length === 0">
                  <td colspan="11" class="bsd-empty">No logs found for this unit in the selected range.</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="bsd-drawer">
          <div class="bsd-drawer-header">
            <div>
              ...
              <span class="bsd-pill" :class="`pill-${drawerRow.status}`" style="margin-left:8px">
          ...
          <template v-else-if="drawerDetail">
            ...
            <div class="bsd-drawer-actions">
              ...
              <button v-if="drawerRow.status === 'missing'"
                class="bsd-btn-secondary"
                @click="triggerMissing(drawerRow)">
                ↻ Trigger Missing Sync
              </button>
              <a :href="`/app/biometric-sync-log/${drawerDetail.name}`"
                target="_blank" class="bsd-btn-ghost">
                Open Full Log ↗
              </a>
            </div>
...
const API = 'biometric_integration.biometric_integration.page.biometric_sync_dashboard.biometric_sync_dashboard.'
...
function statusLabel(s) {
  return { success: 'OK', warning: 'Warning', error: 'Error', pending: 'Queued', missing: 'Missing', empty: 'Empty' }[s] || s
}
...
const filteredRows = computed(() => {
  ...
  switch (activeFilter.value) {
    case 'error':       rows = rows.filter(r => r.status === 'error');                  break
    case 'success':     rows = rows.filter(r => r.status === 'success');                break
    case 'unmapped':    rows = rows.filter(r => (r.unmapped_employees || 0) > 0);      break
    case 'errored':     rows = rows.filter(r => (r.errored || 0) > 0);                 break
    case 'skipped':     rows = rows.filter(r => (r.skipped || 0) > 0);                 break
    case 'raw_records': rows = rows.filter(r => (r.raw_records || 0) > 0);             break
    case 'total':       rows = rows.filter(r => (r.total || 0) > 0);                   break
    case 'created':     rows = rows.filter(r => (r.created || 0) > 0);                 break
  }
...
async function triggerMissing(row) {
  try {
    const msg = await call('trigger_missing_date_sync', { unit: row.unit, sync_date: syncDate.value })
...
.bsd-header-left   { display: flex; flex-direction: column; gap: 4px; }
.bsd-title-row     { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
.bsd-title         { margin: 0; font-size: 17px; font-weight: 700; color: #1a202c; letter-spacing: -0.01em; }
.bsd-header-sub    { display: flex; align-items: center; gap: 8px; }
.bsd-server-badge  { padding: 2px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; flex-shrink: 0; }
.badge-blue        { background: #e0e7ff; color: #3730a3; }
.badge-purple      { background: #ede9fe; color: #5b21b6; }
.bsd-last-sync     { font-size: 11px; color: #718096; }
.bsd-last-sync-dim { font-style: italic; }

.bsd-header-right  { display: flex; align-items: center; gap: 10px; flex-shrink: 0; }
...
.bsd-date-control:focus-within {
  border-color: #5e64ff; box-shadow: 0 0 0 2px rgba(94,100,255,.12); background: #fff;
}
...
.bsd-date-input {
  border: none; background: transparent; font-size: 12px; color: #2d3748;
  font-weight: 500; padding: 0; outline: none; min-width: 110px;
}
...
.bsd-settings-btn {
  ...
  padding: 0 14px; height: 36px; cursor: pointer; font-size: 12px;
  color: #4a5568; font-weight: 500;
  transition: background 0.15s, border-color 0.15s, color 0.15s;
}
.bsd-settings-btn:hover { background: #f7fafc; border-color: #cbd5e0; color: #2d3748; }
.bsd-settings-btn svg   { flex-shrink: 0; }
...
.bsd-tab:hover  { color: #4a5568; background: #f7fafc; }
.bsd-tab.active { color: #5e64ff; border-bottom-color: #5e64ff; font-weight: 600; background: #fafaff; }

/* ── loading ─────────────────────────────────────────────────────────────────*/
.bsd-loading { display: flex; align-items: center; gap: 12px; padding: 60px 20px; justify-content: center; color: #718096; }
.bsd-spinner {
  width: 20px; height: 20px; border: 2px solid #e2e8f0;
  border-top-color: #5e64ff; border-radius: 50%; animation: spin 0.6s linear infinite;
}
@keyframes spin { to { transform: rotate(360deg); } }

/* ── kpi row ─────────────────────────────────────────────────────────────────*/
.bsd-kpi-row {
  display: grid; grid-template-columns: repeat(6, 1fr); gap: 12px; margin-bottom: 6px;
}
@media (max-width: 1000px) { .bsd-kpi-row { grid-template-columns: repeat(3, 1fr); } }
@media (max-width: 600px)  { .bsd-kpi-row { grid-template-columns: repeat(2, 1fr); } }
...
.bsd-kpi-card:hover   { box-shadow: 0 2px 8px rgba(0,0,0,.08); border-color: #cbd5e0; }
.bsd-kpi-card.active  { border-color: #5e64ff; box-shadow: 0 0 0 2px rgba(94,100,255,.15); }
.bsd-kpi-card.warn    { border-left: 3px solid #f6ad55; }
.bsd-kpi-card.warn.active   { border-color: #f6ad55; box-shadow: 0 0 0 2px rgba(246,173,85,.2); }
.bsd-kpi-card.error   { border-left: 3px solid #fc8181; }
.bsd-kpi-card.error.active  { border-color: #fc8181; box-shadow: 0 0 0 2px rgba(252,129,129,.2); }
.bsd-kpi-card.healthy { border-left: 3px solid #68d391; }
.bsd-kpi-card.healthy.active { border-color: #68d391; box-shadow: 0 0 0 2px rgba(104,211,145,.2); }

.bsd-kpi-icon {
  width: 36px; height: 36px; border-radius: 8px;
  display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: 700; flex-shrink: 0;
}
.bsd-kpi-icon.punch    { background: #eff6ff; color: #3b82f6; }
.bsd-kpi-icon.created  { background: #f0fdf4; color: #16a34a; }
.bsd-kpi-icon.skipped  { background: #f5f3ff; color: #7c3aed; }
.bsd-kpi-icon.unmapped { background: #fffbeb; color: #d97706; }
.bsd-kpi-icon.api-err  { background: #fef2f2; color: #dc2626; }
.bsd-kpi-icon.loc-ok   { background: #f0fdf4; color: #16a34a; }

.bsd-kpi-body  { min-width: 0; }
.bsd-kpi-value { font-size: 20px; font-weight: 700; line-height: 1.2; }
.bsd-kpi-total { font-size: 14px; font-weight: 400; color: #718096; }
.bsd-kpi-label { font-size: 11px; font-weight: 600; color: #4a5568; margin-top: 2px; }
.bsd-kpi-sub   { font-size: 10px; color: #a0aec0; }

/* ── kpi legend ──────────────────────────────────────────────────────────────*/
.bsd-kpi-legend { display: flex; align-items: center; gap: 16px; margin-bottom: 16px; flex-wrap: wrap; }
.bsd-legend-group { font-size: 10px; color: #a0aec0; }
.bsd-legend-group.records   { color: #3b82f6; }
.bsd-legend-group.employees { color: #d97706; }
.bsd-filter-badge {
  background: #eef2ff; color: #4338ca; padding: 2px 10px; border-radius: 10px;
  font-size: 11px; cursor: pointer; font-weight: 500;
}
.bsd-filter-badge:hover { background: #e0e7ff; }

/* ── card shell ──────────────────────────────────────────────────────────────*/
.bsd-card { background: #fff; border: 1px solid #e8edf2; border-radius: 10px; margin-bottom: 16px; overflow: hidden; }
.bsd-card-header {
  display: flex; align-items: center; justify-content: space-between;
  padding: 12px 16px; border-bottom: 1px solid #f0f4f8;
}
.bsd-card-header h4 { margin: 0; font-size: 14px; font-weight: 600; }
.bsd-table-meta     { font-size: 11px; color: #718096; }

/* ── table ───────────────────────────────────────────────────────────────────*/
.bsd-table-wrap { overflow-x: auto; }
...
.bsd-table thead th {
  ...
  color: #718096; background: #f8fafc; border-bottom: 1px solid #e8edf2;
  white-space: nowrap; user-select: none;
}
.bsd-table thead th.sortable       { cursor: pointer; }
.bsd-table thead th.sortable:hover { color: #4a5568; }
.bsd-table thead th.num-col        { text-align: right; }

.bsd-table tbody td {
  padding: 9px 12px; border-bottom: 1px solid #f7f9fb; vertical-align: middle;
}
.bsd-table tbody td.num-col        { text-align: right; font-variant-numeric: tabular-nums; }
.bsd-table tbody tr:last-child td  { border-bottom: none; }
.bsd-table tbody tr:hover          { background: #fafbff; }
.bsd-row-missing td                { opacity: 0.6; }
...
.bsd-date-cell { white-space: nowrap; font-weight: 500; }

.bsd-dot { display: inline-block; width: 8px; height: 8px; border-radius: 50%; }
.dot-success { background: #48bb78; }
.dot-warning { background: #f6ad55; }
.dot-error   { background: #fc8181; }
.dot-pending { background: #90cdf4; }
.dot-missing { background: #e2e8f0; border: 1px solid #cbd5e0; }
.dot-empty   { background: #cbd5e0; }

.bsd-pill { padding: 2px 8px; border-radius: 8px; font-size: 10px; font-weight: 600; display: inline-block; }
.pill-success { background: #f0fff4; color: #22543d; }
.pill-warning { background: #fffaf0; color: #744210; }
.pill-error   { background: #fff5f5; color: #742a2a; }
.pill-pending { background: #ebf8ff; color: #2a4365; }
.pill-missing { background: #f7fafc; color: #718096; }
.pill-empty   { background: #f1f5f9; color: #64748b; }

.bsd-unit-cell  { min-width: 130px; }
.bsd-unit-name  { font-weight: 500; display: block; }
.bsd-log-link   { font-size: 10px; color: #5e64ff; text-decoration: none; display: block; }
.bsd-log-link:hover { text-decoration: underline; }

.bsd-sort-icon { font-size: 8px; margin-left: 3px; display: inline-flex; flex-direction: column; line-height: 1; }

.bsd-nil       { color: #cbd5e0; }
.num-warn      { color: #d97706; font-weight: 600; }
.num-error     { color: #e53e3e; font-weight: 600; }
.num-unit      { font-size: 9px; color: #a0aec0; margin-left: 2px; }

.bsd-prog-wrap    { display: flex; align-items: center; gap: 6px; }
.bsd-prog-bar     { flex: 1; height: 6px; background: #edf2f7; border-radius: 3px; overflow: hidden; }
.bsd-prog-fill    { height: 100%; border-radius: 3px; transition: width 0.3s; }
.prog-green       { background: #48bb78; }
.prog-yellow      { background: #f6ad55; }
.prog-red         { background: #fc8181; }
.bsd-prog-pct     { font-size: 10px; color: #718096; min-width: 32px; text-align: right; }
.bsd-prog-caption { font-size: 9px; color: #a0aec0; margin-top: 1px; }

.bsd-action-btn {
  padding: 3px 8px; border-radius: 4px; font-size: 10px; font-weight: 500;
  cursor: pointer; border: 1px solid; margin-right: 4px; text-decoration: none; display: inline-block;
}
.bsd-action-btn.retry         { background: #fff5f5; color: #e53e3e; border-color: #fed7d7; }
.bsd-action-btn.retry:hover   { background: #fed7d7; }
.bsd-action-btn.trigger       { background: #ebf8ff; color: #2b6cb0; border-color: #bee3f8; }
.bsd-action-btn.trigger:hover { background: #bee3f8; }
.bsd-action-btn.detail        { background: #f7fafc; color: #4a5568; border-color: #e2e8f0; }
.bsd-action-btn.detail:hover  { background: #edf2f7; }

.bsd-empty { text-align: center; padding: 40px 20px; color: #a0aec0; }

/* ── charts row ──────────────────────────────────────────────────────────────*/
.bsd-charts-row { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
@media (max-width: 768px) { .bsd-charts-row { grid-template-columns: 1fr; } }
.bsd-chart-card  { padding: 0; }
.bsd-chart-sub   { font-size: 11px; color: #a0aec0; }
.bsd-chart-area  { padding: 8px 12px 4px; min-height: 200px; }
.bsd-chart-empty { padding: 40px; text-align: center; color: #a0aec0; font-size: 12px; }
...
.bsd-failure-list  { padding: 16px; display: flex; flex-direction: column; gap: 10px; }
.bsd-failure-row   { display: flex; align-items: center; gap: 10px; }
.bsd-failure-label { min-width: 160px; font-size: 11px; color: #4a5568; text-align: right; }
.bsd-failure-bar-wrap { flex: 1; height: 8px; background: #edf2f7; border-radius: 4px; overflow: hidden; }
.bsd-failure-bar   { height: 100%; background: #fc8181; border-radius: 4px; transition: width 0.3s; }
.bsd-failure-count { font-size: 11px; font-weight: 600; min-width: 30px; }
...
@media (max-width: 800px) {
  .bsd-unit-filter-bar { grid-template-columns: 1fr 1fr; }
  .bsd-load-btn { grid-column: span 2; }
}
.bsd-filter-group { display: flex; flex-direction: column; gap: 5px; }
.bsd-filter-group label { font-size: 11px; color: #718096; font-weight: 600; letter-spacing: 0.02em; }
.bsd-select {
  border: 1px solid #e2e8f0; border-radius: 6px; padding: 6px 10px;
  font-size: 12px; background: #fff; width: 100%; appearance: auto; height: 32px;
}
.bsd-select:focus { outline: none; border-color: #5e64ff; box-shadow: 0 0 0 2px rgba(94,100,255,.15); }
/* Align the date inputs in Tab 2 filter to match the select height */
.bsd-unit-filter-bar .bsd-date-input { width: 100%; height: 32px; padding: 6px 8px; }
.bsd-load-btn { white-space: nowrap; height: 32px; padding: 0 20px; }
...
.bsd-sum-stat {
  ...
  padding: 12px 18px; border-radius: 8px; background: #f8fafc;
  min-width: 96px; flex: 1;
  border: 1px solid #edf2f7;
}
.bsd-sum-stat.ok    { background: #f0fff4; border-color: #c6f6d5; }
.bsd-sum-stat.warn  { background: #fffbeb; border-color: #feebc8; }
.bsd-sum-stat.error { background: #fff5f5; border-color: #fed7d7; }
.bsd-sum-value { font-size: 22px; font-weight: 700; line-height: 1.1; color: #2d3748; }
.bsd-sum-stat.ok    .bsd-sum-value { color: #276749; }
.bsd-sum-stat.warn  .bsd-sum-value { color: #c05621; }
.bsd-sum-stat.error .bsd-sum-value { color: #c53030; }
.bsd-sum-label { font-size: 10px; color: #718096; text-align: center; font-weight: 500; white-space: nowrap; }

.bsd-tab2-hint {
  text-align: center; padding: 64px 24px; color: #a0aec0; font-size: 13px;
  background: #fff; border: 1px dashed #e2e8f0; border-radius: 10px;
  line-height: 1.6;
}
.bsd-tab2-hint strong { color: #5e64ff; }
...
.bsd-stat-value.ok     { color: #22543d; }
.bsd-stat-value.warn   { color: #d97706; }
.bsd-stat-value.error  { color: #c53030; }
...
.bsd-btn-primary:hover    { background: #4b52e0; }
.bsd-btn-primary:disabled { background: #a0aec0; cursor: not-allowed; }
.bsd-btn-secondary {
  background: #f7fafc; color: #4a5568; border: 1px solid #e2e8f0; border-radius: 6px;
  padding: 8px 16px; cursor: pointer; font-size: 12px; font-weight: 500; text-align: center;
}
.bsd-btn-secondary:hover { background: #edf2f7; }
