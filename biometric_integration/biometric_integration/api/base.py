...
class BiometricApiClient:
    ...
    def __init__(self):
        ...
        self.base_url = (self.settings.endpoint_url.rstrip("/") + WEB_URI_BIO_SERVER) if self.settings.server_type == "Bio Server" else (self.settings.endpoint_url.rstrip("/") + WEB_URI_ETRACKER_LITE)
        ...
        } if self.settings.server_type == "Bio Server" else {
    ...
    def get_device_logs(self):
        ...
        if server_type == "Bio Server":
            ...
            try:
                ...
                body = self._build_etime_server_envelope(log_date , frappe.utils.now_datetime(), self.serial_no, self.username, self.password)

                log = create_biometric_log(
                    method=self.get_device_logs.__name__, request_data=body, make_new=True
                )
                frappe.flags.request_id = log.name
                response = requests.post(self.base_url, data=body, headers={**self.headers , **{"SOAPAction": "http://tempuri.org/GetTransactionsLog"}}, timeout=30)
    ...
    def retry_get_device_logs(self, payload , request_id):
        ...
        if "GetTransactionsLog" not in payload:
            try:
                ...
                response = requests.post(self.base_url, data=payload, headers={**self.headers , **{"SOAPAction": "http://tempuri.org/DeviceLogs"}}, timeout=30)
            ...
            try:
                frappe.flags.request_id = request_id
                response = requests.post(self.base_url, data=payload, headers={**self.headers , **{"SOAPAction": "http://tempuri.org/GetTransactionsLog"}}, timeout=30)
...
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <soap:Body>
    ...
    def _build_etime_server_envelope(self,from_dt: datetime, to_dt: datetime, serial: str, username: str, password: str) -> str:
